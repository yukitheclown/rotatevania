...
IVec2 Math_GetDistanceIRect(IRect r1, IRect r2){
    ...
    return (IVec2){ Math_GetDistanceInt(r1.x, r1.x+r1.w, r2.x, r2.x+r2.w), Math_GetDistanceInt(r1.y, r1.y+r1.h, r2.y, r2.y+r2.h) };
}
...
int Math_IRectCheckCollisionRay(IRect r, Ray ray){
    ...
    points[0] = (IVec3){r.x, r.y, r.z};
    points[1] = (IVec3){r.x+r.w, r.y, r.z};
    points[2] = (IVec3){r.x+r.w, r.y+r.h, r.z};
    points[3] = (IVec3){r.x, r.y+r.h, r.z};
    norms[0] = (IVec3){0,1,0};
    norms[1] = (IVec3){1,0,0};
    norms[2] = (IVec3){0,-1,0};
    norms[3] = (IVec3){-1,0,0};

    IVec3 rayLine = (IVec3){ray.line.x, ray.line.y, 0};
    IVec3 rayPos = (IVec3){ray.pos.x, ray.pos.y, 0};

    int k;
    for(k = 0; k < 4; k++){
        
        int d = Math_IVec3Dot(Math_IVec3SubIVec3(points[k], rayPos), norms[k]) / Math_IVec3Dot(rayLine, norms[k]);
        if(d < 0) continue;


        IVec3 collisionPoint = Math_IVec3AddIVec3(rayPos, Math_IVec3MultInt(rayLine,d));
...
Vec2 Math_LerpVec2(Vec2 a1, Vec2 a2, float t){
    return Math_Vec2AddVec2(Math_Vec2MultFloat(a1,(1.0-t)), Math_Vec2MultFloat(a2, t));
}
